...
function SummaryBand() {
  return (
    <section className="overflow-hidden rounded-xl border border-border/60 bg-background/80 shadow-sm">
      <div className="grid divide-y divide-border/60 md:grid-cols-2 lg:grid-cols-4 md:divide-y-0 md:divide-x">
        {summary.map((item) => (
          <div key={item.label} className="space-y-3 px-4 py-4 sm:px-5">
...
export function DashboardPlaceholder() {
  return (
    <section className="page-reveal flex w-full max-w-none flex-col gap-5 px-4 py-6">
        ...
        <div className="space-y-2">
          <div className="space-y-1">
            ...
            <p className="max-w-2xl text-sm leading-5 text-muted-foreground">
              Overview of key metrics, performance trends, and recent activities
            </p>
          </div>
        </div>
      ...
      <div className="grid gap-5 md:grid-cols-1 lg:grid-cols-2">
        <section className="min-w-0 rounded-xl border border-border/60 bg-background/80 shadow-sm px-4 py-4 sm:px-5">
          <div className="flex flex-col gap-2 border-b border-border/60 pb-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <h2 className="text-base font-semibold tracking-tight text-foreground">Commercial trend</h2>
              <p className="mt-1 text-xs text-muted-foreground">
                Monthly performance across revenue and completed orders
              </p>
            </div>
            <Badge variant="outline" size="sm">Last 6 months</Badge>
          </div>

          <div className="pt-5">
            <ChartContainer config={trendConfig} className="aspect-auto h-[260px] w-full max-w-full overflow-hidden">
        ...
        <section className="min-w-0 overflow-hidden rounded-xl border border-border/60 bg-background/80 shadow-sm px-4 py-4 sm:px-5">
          <div className="border-b border-border/60 pb-4">
            <h2 className="text-base font-semibold tracking-tight text-foreground">Acquisition mix</h2>
            <p className="mt-1 text-xs text-muted-foreground">
          ...
          <div className="pt-5">
            <ChartContainer config={channelConfig} className="aspect-auto h-[200px] w-full max-w-full overflow-hidden">
              <BarChart data={channelData} margin={{ left: 4, right: 4, top: 4 }}>
                ...
                <Bar dataKey="share" fill="var(--color-share)" radius={8} />
              </BarChart>
            </ChartContainer>

            <div className="mt-4 space-y-3">
              {channelData.map((item) => (
                <div key={item.source} className="flex items-center justify-between border-b border-dashed border-border/60 pb-2 last:border-0 last:pb-0">
      ...
      <div className="grid gap-5 md:grid-cols-1 lg:grid-cols-2">
        <section className="rounded-xl border border-border/60 bg-background/80 shadow-sm px-4 py-4 sm:px-5">
          <div className="border-b border-border/60 pb-4">
            <h2 className="text-base font-semibold tracking-tight text-foreground">Pipeline</h2>
            <p className="mt-1 text-xs text-muted-foreground">
          ...
          <div className="divide-y divide-border/60 pt-3">
            {pipeline.map((item) => (
              <div key={item.name} className="grid gap-2 py-3 sm:grid-cols-[minmax(0,1.2fr)_100px_100px_60px] sm:items-center sm:gap-3">
                <div>
        ...
        <section className="rounded-xl border border-border/60 bg-background/80 shadow-sm px-4 py-4 sm:px-5">
          <div className="border-b border-border/60 pb-4">
            <h2 className="text-base font-semibold tracking-tight text-foreground">Recent changes</h2>
            <p className="mt-1 text-xs text-muted-foreground">
          ...
          <div className="space-y-3 pt-4">
            {activity.map((item) => (
              <div key={item.title} className="space-y-1 border-b border-dashed border-border/60 pb-3 last:border-0 last:pb-0">
